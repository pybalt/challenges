        ...
        if tool_endpoint is not None:
            ...
            for content_block in response_params:
                ...
                if content_block["type"] == "tool_use":
                    tasks.append(asyncio.create_task(_call_remote(tool_endpoint, content_block, tool_version)))
            for future in asyncio.as_completed(tasks):
                result, tool_use_id = await future
                tool_result_content.append(_make_api_tool_result(result, tool_use_id))
...
async def _call_remote(tool_endpoint: str, content_block: dict, tool_version: str) -> tuple[ToolResult, str]:
    try:
        async with aiohttp.ClientSession() as session:
            async with session.post(
                tool_endpoint + "/execute_tool",
                json={"tool_name": content_block["name"],
    ...
    return result, content_block["id"]
