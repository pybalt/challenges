...
# Scalable Backend for Computer Use Agent

This extends the original demo with a FastAPI backend for session management, real-time streaming via WebSocket, VNC integration, and database persistence.

## Setup Instructions

1. Build the tool container image:
   ```
   docker build -t computer-use-demo-latest .
   ```
...
3. Run the backend (requires Docker socket for managing containers):
   ```
   docker run -p 8000:8000 --volume /var/run/docker.sock:/var/run/docker.sock backend-latest
   ```

4. Open the frontend in your browser: http://localhost:8000/

## Features

- **Session Creation**: POST /sessions to create a new session, spinning up a dedicated container.
- **Message Sending**: POST /sessions/{id}/messages to send a message and trigger the agent loop.
- **Real-time Updates**: Connect to ws://localhost:8000/ws/{id} for live progress.
- **Chat History**: GET /sessions/{id}/messages to retrieve persisted history.
- **VNC**: GET /sessions/{id}/vnc for the noVNC URL.
- **Delete Session**: DELETE /sessions/{id} to stop and remove the container.

## Notes

- Replace API key in the frontend script or pass dynamically.
- For production, secure the API key handling and add authentication.
- The backend uses SQLite for persistence and manages containers via docker-py.

For more details, see the code in backend/app.py and the original demo documentation.
